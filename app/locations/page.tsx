...
export default function LocationsManager() {
  ...
  // NEU: Stammbaum-Sortierung für die Dropdowns & Listen
  ...
  const handleAddLocation = async (e: React.FormEvent) => {
    ...
    try {
      let imageUrl = "";
      if (imageFile) {
        const fileRef = ref(storage, `locations/${Date.now()}_${imageFile.name}`);
      ...
      await addDoc(collection(db, 'locations'), {
        ...
        code: finalCode, // Für spätere Barcodes/Labels
        imageUrl: imageUrl, // Foto vom Regal/Kiste
          ...
          <form onSubmit={handleAddLocation} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 outline-none" required />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1">Label-Code (Optional)</label>
                <input type="text" value={locCode} onChange={(e) => setLocCode(e.target.value)} placeholder="Wird auto-generiert..." className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 outline-none font-mono text-sm" />
              </div>
            </div>
            
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1">Steht innerhalb von...</label>
              <select value={parentLocId} onChange={(e) => setParentLocId(e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 outline-none">
                <option value="">-- Haupt-Lagerort (Kein Eltern-Ort) --</option>
                {hierarchicalLocs.map(loc => (
                  <option key={loc.id} value={loc.id}>
                    {/* OPTISCHE EINRÜCKUNG IM DROPDOWN */}
            ...
            <div>
              ...
              <input type="file" accept="image/*" capture="environment" onChange={(e) => e.target.files && setImageFile(e.target.files[0])} className="text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-slate-100 file:text-slate-700" />
            </div>

            <button type="submit" disabled={isSaving} className="w-full bg-slate-800 text-white font-bold py-3 rounded-xl hover:bg-slate-900 transition mt-2">
              {isSaving ? 'Speichere...' : '+ Ort anlegen'}
            </button>
          </form>
        </div>

        {/* LISTE DER BESTEHENDEN ORTE (Hierarchisch) */}
        <div className="space-y-3">
          ...
          {hierarchicalLocs.map(loc => (
            <div key={loc.id} className="flex items-center bg-white p-3 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden" style={{ marginLeft: `${loc.depth * 20}px` }}>
              ...
              {/* Farbiger Balken für Hauptorte */}
              {loc.depth === 0 && <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-slate-800"></div>}
              {loc.depth > 0 && <div className="text-slate-300 mr-2 ml-1">↳</div>}
              ...
              <div className="flex-1 min-w-0">
                <p className={`font-bold text-slate-800 ${loc.depth === 0 ? 'text-lg' : 'text-md'}`}>{loc.name}</p>
