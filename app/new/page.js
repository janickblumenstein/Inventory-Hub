...
import { useRouter } from 'next/navigation'; // <-- Das ist der neue Import für die Weiterleitung
...
export default function NewItem() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      // WICHTIG: Hier werfen wir dich jetzt sofort zurück aufs Dashboard!
      router.push('/'); 
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          {previewUrl && (
            <div className="relative inline-block mt-4 border-2 border-slate-200 rounded-lg overflow-hidden">
              <img src={previewUrl} alt="Vorschau" onClick={handleImageClick} className="max-w-full h-auto cursor-crosshair" />
              {tagCoords && (
                <div 
          ...
          <button type="submit" disabled={isSaving} className="w-full bg-orange-600 text-white font-bold py-3 rounded-lg hover:bg-orange-700 disabled:opacity-50 mt-8">
            {isSaving ? 'Speichert...' : 'Item mit Foto speichern'}
          </button>
        </form>
      </div>
    </div>
