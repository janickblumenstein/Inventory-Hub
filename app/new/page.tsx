...
function NewItemForm() {
  ...
  // NEU: Lade-Status für die Online-Datenbank
  const [isFetchingApi, setIsFetchingApi] = useState(false);

  const [selectedLocationImage, setSelectedLocationImage] = useState<string | null>(null);
  const [tagCoords, setTagCoords] = useState<{x: number, y: number} | null>(null);

  // Lade Lagerorte beim Start
  useEffect(() => {
    const fetchLocations = async () => {
      const querySnapshot = await getDocs(collection(db, 'locations'));
      const locs: any[] = [];
  ...
  // NEU: Die Magie! Suche im Internet nach dem Barcode
  useEffect(() => {
    if (ean) {
      const fetchEanData = async () => {
        setIsFetchingApi(true);
        try {
          // Wir nutzen die kostenlose Test-API von upcitemdb
          const response = await fetch(`https://api.upcitemdb.com/prod/trial/lookup?upc=${ean}`);
          const data = await response.json();

          if (data && data.items && data.items.length > 0) {
            // Treffer! Wir setzen den Namen automatisch
            const foundTitle = data.items[0].title;
            setName(foundTitle);
            
            // Kleine UX-Magie: Wir triggern direkt unsere smarte Tag-Erkennung für den neuen Namen!
            generateSmartTagsFor(foundTitle);
          }
        } catch (error) {
          console.error("Fehler beim Abrufen der EAN-Daten:", error);
          // Fällt lautlos fehl, Nutzer kann den Namen einfach manuell eintragen
        } finally {
  ...
  // Wir lagern die Tag-Logik leicht aus, damit wir sie auch beim automatischen API-Fetch nutzen können
  ...
  return (
    <form onSubmit={handleSave} className="space-y-6">
      ...
      {/* Anzeige der eingescannten EAN MIT API-Status */}
      {ean && (
      ...
      <div>
        ...
        {!isFetchingApi && ean && name && (
          <p className="text-xs text-green-600 mt-1">✓ Name erfolgreich geladen oder manuell eingetragen.</p>
        )}
      </div>
